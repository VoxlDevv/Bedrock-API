...
class Database {
    ...
    constructor(name) {
        ...
        if (this.DB_SAVED_NAMES.includes(name))
            this.error.CustomError("Database", "constructor", `Database with name ${name} already exist`);
    ...
    async set(key, value) {
        if (value.length >= 32000)
            this.error.CustomError("Database", "set", "Value length is too much, limit character is 32000 or 32k");
        const [encryptKey, encryptValue] = [key, value].map((item) => Formatter.EncryptText(JSON.stringify(item)));
        this.delete(key);
        await null;
    ...
    async delete(key) {
        if (!this.hasKey(key))
            return undefined;
        const [encryptKey, encryptValue] = [key, this.RESTORED_DATA.get(key)].map((item) => Formatter.EncryptText(JSON.stringify(item)));
        await null;
        new ChatClass().runCommand(`scoreboard players reset "[${encryptKey},${encryptValue}]" "DB_${this.DB_NAME}"`);
